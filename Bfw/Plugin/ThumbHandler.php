...
// $t->setSrcImg("upload/test.jpg");
// $t->setDstImg("upload/new_test.jpg");
// $t->setMaskImg("upload/12.gif");
// $t->setMaskPosition(1);
// $t->setMaskImgPct(60); //透明度
// $t->setDstImgBorder(4,"#ffffff");
// 指定缩放比例
// $t->createImg(301,220);
...
// $t->setRectangleCut(301, 220);// 裁切尺寸
// $t->setDstImg("upload/new_test.jpg");
// $t->createImg(301,220);
...
/*
 * 使用方法: 自动裁切: 程序会按照图片的尺寸从中部裁切最大的正方形，并按目标尺寸进行缩略 $t->setSrcImg("img/test.jpg");
 * $t->setCutType(1);//这一句就OK了 $t->setDstImg("tmp/new_test.jpg");
 * $t->createImg(60,60); 手工裁切: 程序会按照指定的位置从源图上取图 $t->setSrcImg("img/test.jpg");
 * $t->setCutType(2);//指明为手工裁切 $t->setSrcCutPosition(100, 100);// 源图起点坐标
 * $t->setRectangleCut(300, 200);// 裁切尺寸 $t->setDstImg("tmp/new_test.jpg");
 * $t->createImg(300,200);
 */
class ThumbHandler {
	var $debugmode = false;
	...
	function createratioImg($w) {
		if ($this->src_w < $w) {
			$w = $this->src_w;
		}
		// $this->src_w
		// echo $this->src_w;
		$_newheight = round ( ($this->src_h / $this->src_w) * $w );
		
		$this->createImg ( $w, $_newheight );
	}
	/**
	 * 创建图片,主函数
	 *
	...
	function createImg($a, $b = null) {
		...
		if (2 == $num) {
			...
			if (0 == $w) {
				// $w=600;
				$this->ErrForHttp ( "目标宽度不能为0" );
			}
			if (0 == $h) {
				// $h=400;
				$this->ErrForHttp ( "目标高度不能为0" );
			}
	...
	function _output() {
		...
		if (function_exists ( $func_name )) {
			...
			// $im = imagecreatefrompng("upload/test.png");
			// imagepng($im);
			// var_dump($this->img_display_quality);
	...
	function _setNewImgSize($img_w, $img_h = null) {
		...
		if (2 == $num) {
			...
			if ($fill_w < 0 || $fill_h < 0) {
				$this->ErrForHttp ( "图片边框过大，已超过了图片的宽度" );
			}
			...
			switch ($this->cut_type) {
					...
					if ($rate_w >= 1 && $rate_h >= 1) {
						if ($this->src_w > $this->src_h) {
							...
							if($this->src_w/$this->src_h>$fill_w/$fill_h){
							    $this->copy_w = round($this->src_h*($fill_w/$fill_h));
							    $this->copy_h = $this->src_h;
							}else{
								$this->copy_w = $this->src_w;
								$this->copy_h = round($this->src_w/($fill_w/$fill_h));
							}
							LogR("OLD".$this->src_w."*".$this->src_h."COPY".$this->copy_w."*".$this->copy_h."RECT".$fill_w."*".$fill_h);
				...
				case 3 :
					...
					$this->copy_w = $this->src_w;
					$this->copy_h = $this->src_h;
					$_img_width = $img_w + $this->img_border_size * 2;
					$_img_height = $img_h + $this->img_border_size * 2;
					// 目标尺寸
					$this->dst_w = $_img_width;
					$this->dst_h = $_img_height;
					
					// 目标文件起始坐标
					if ($_img_width > $this->fill_w) {
						$this->start_x = ($_img_width - $this->fill_w) / 2 + $this->img_border_size;
						$this->start_y = $this->img_border_size;
					} else if ($_img_height > $this->fill_h) {
						$this->start_x = $this->img_border_size;
						$this->start_y = ($_img_height - $this->fill_h) / 2 + $this->img_border_size;
					} else {
						$this->start_x = $this->img_border_size;
						$this->start_y = $this->img_border_size;
					}
	...
	function _checkMaskValid() {
		if ($this->mask_w + $this->mask_offset_x > $this->src_w || $this->mask_h + $this->mask_offset_y > $this->src_h) {
			return false;
			// $this->ErrForHttp ( "水印图片尺寸大于原图，请缩小水印图" );
		} else {
			return true;
		}
	}
	...
	function _getImgType($file_path) {
		...
		if (file_exists ( $file_path )) {
			...
			$this->ErrForHttp ( "文件不存在,不能取得文件类型!" );
		}
	}
 ...
 * 创建bmp格式图片
 *
 * ription: create Bitmap-File with GD library
 *
 ...
 *        	如果要另存为文件，请指定文件名，为空则直接在浏览器输出
 * @param integer $bit
 *        	图像质量(1、4、8、16、24、32位)
 * @param integer $compression
 *        	压缩方式，0为不压缩，1使用RLE8压缩算法进行压缩
 *        	
 * @return integer
 */
