...
function pageInit()
{
	...
	editor=$('#elm1').xheditor({skin:'nostyle',internalStyle:true,inlineStyle:true,tools:'mfull',plugins:allPlugin,loadCSS:'<style>pre{	border-left: 2px dashed #888;background: #FFFFE0;margin-left: 20px;}</style>',shortcuts:{'ctrl+enter':submitForm}});
...
<body>
  ...
  <li><a href="#">首页</a></li>
    ...
    <ul class="forminfo">
    <li><label>标题</label><input name="title" type="text" value="<?=$itemdata['title']?>" class="dfinput" /><i></i></li>
    <li><label>类别</label><input name="classname" type="text" class="dfinput" value="<?=$itemdata['classname']?>" /><i> </i></li>
    <li style="height:200px;clear:both;"><label>内容</label>
    <textarea id="elm1" name="content" rows="12" cols="10" style="width:70%;"><?=HtmlUtil::Encode($itemdata['content'])?></textarea> </li>
