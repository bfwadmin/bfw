...
class PagerUtil
{
    ...
    public static function _GenPageData($sums = 0, $page = 0, $pagesize = 10, $shownum = 3, $url = URL, $pageid = "page", $routetype = ROUTETYPE)
    {
        $page = intval($page);
        $_pageData = array();
        ...
        if ($pages > 0) {
            ...
            if ($page > 0)
                $_everypageData[] = array(
                    "urltype" => "first",
                    "url" => self::GetPageUrl($url, 0, $pageid, $routetype)
                );
            if($page>=1)
                $_everypageData[] = array(
                    "urltype" => "prev",
                    "url" => self::GetPageUrl($url, $page-1, $pageid, $routetype)
                );
            ...
            for ($i = $startpage; $i <= $endpage; $i ++) {
                if ($i == $page)
                    $_everypageData[] = array(
                        "urltype" => "page",
                        "url" => self::GetPageUrl($url, $i, $pageid, $routetype),
                        "pagenum" => $i + 1,
                        "current" => true
                    );
                else
            ...
            if ($endpage < $pages)
                ...
                if ($page < $pages)
                    $_everypageData[] = array(
                        "urltype" => "last",
                        "url" => self::GetPageUrl($url, $pages, $pageid, $routetype)
                    );
                if($page+1<=$pages)
                    $_everypageData[] = array(
                        "urltype" => "next",
