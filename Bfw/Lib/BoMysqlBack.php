...
//     }
    ...
    private function _insert_db_structure($db)
    {
        $sql = '';
        $sql .= "--" . $this->ds;
        $sql .= "-- 创建数据库" . $db . $this->ds;
        $sql .= "--" . $this->ds . $this->ds;
        // 如果存在则删除表
        $sql .= "CREATE DATABASE `" . $db . "`  DEFAULT CHARACTER  SET `".DB_CHARACTER."`  COLLATE  `".DB_COLLATE ."` " . $this->sqlEnd . $this->ds;
        $sql .= "USE `" . $db . "` " . $this->sqlEnd . $this->ds;
        return $sql;
    }
    ...
    private function _write_filename($sql, $filename)
    {
        $re = true;
