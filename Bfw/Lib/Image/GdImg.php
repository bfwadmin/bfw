...
class GdImg {
	 ...
	 *        	缩略图裁剪类型
	 */
	public function thumb($width, $height, $type = BoImg::IMAGE_THUMB_SCALE) {
		if (empty ( $this->img ))
		...
		switch ($type) {
			...
			/* 左上角裁剪 */
			case BoImg::IMAGE_THUMB_NORTHWEST :
				// 计算缩放比例
				$scale = max ( $width / $w, $height / $h );
			...
			/* 右下角裁剪 */
			case BoImg::IMAGE_THUMB_SOUTHEAST :
				// 计算缩放比例
				$scale = max ( $width / $w, $height / $h );
	 ...
	 *        	水印透明度
	 */
	public function water($source, $locate = BoImg::IMAGE_WATER_SOUTHEAST, $alpha = 80) {
		// 资源检测
		if (empty ( $this->img ))
		...
		switch ($locate) {
			/* 右下角水印 */
			case BoImg::IMAGE_WATER_SOUTHEAST :
				$x = $this->info ['width'] - $info [0];
				$y = $this->info ['height'] - $info [1];
				break;
			
			/* 左下角水印 */
			case BoImg::IMAGE_WATER_SOUTHWEST :
				$x = 0;
				$y = $this->info ['height'] - $info [1];
				break;
			
			/* 左上角水印 */
			case BoImg::IMAGE_WATER_NORTHWEST :
				$x = $y = 0;
				break;
			
			/* 右上角水印 */
			case BoImg::IMAGE_WATER_NORTHEAST :
				$x = $this->info ['width'] - $info [0];
				$y = 0;
				break;
			
			/* 居中水印 */
			case BoImg::IMAGE_WATER_CENTER :
				$x = ($this->info ['width'] - $info [0]) / 2;
				$y = ($this->info ['height'] - $info [1]) / 2;
				break;
			
			/* 下居中水印 */
			case BoImg::IMAGE_WATER_SOUTH :
				$x = ($this->info ['width'] - $info [0]) / 2;
				$y = $this->info ['height'] - $info [1];
				break;
			
			/* 右居中水印 */
			case BoImg::IMAGE_WATER_EAST :
				$x = $this->info ['width'] - $info [0];
				$y = ($this->info ['height'] - $info [1]) / 2;
				break;
			
			/* 上居中水印 */
			case BoImg::IMAGE_WATER_NORTH :
				$x = ($this->info ['width'] - $info [0]) / 2;
				$y = 0;
				break;
			
			/* 左居中水印 */
			case BoImg::IMAGE_WATER_WEST :
				$x = 0;
				$y = ($this->info ['height'] - $info [1]) / 2;
				break;
	 ...
	 *        	文字倾斜角度
	 */
	public function text($text, $font, $size, $color = '#00000000', $locate = BoImg::IMAGE_WATER_SOUTHEAST, $offset = 0, $angle = 0) {
		// 资源检测
		if (empty ( $this->img ))
