...
function createTree(p_div, p_backColor, p_contextMenu) {
	var tree = {
		...
		// p_expanded: True or false, indicating wether the node starts expanded
		// or not;
		// p_icon: Relative path to the icon displayed with the node. Set null
		// if the node has no icon;
		// p_parentNode: Reference to the parent node. Set null to create the
		// node on the root;
		// p_tag: Tag is used to store additional information on the node. All
		// node attributes are visible when programming events and context menu
		// actions;
		// p_contextmenu: Name of the context menu, which is one of the
		// attributes of the p_contextMenu object created with the tree;
		createNode : function(p_text, p_expanded, p_icon, p_parentNode, p_tag,
				p_contextmenu) {
			v_tree = this;
			node = {
				...
				// /// Expanding or collapsing the node, depending on the
				// expanded value
				...
				// /// Expanding or collapsing the node, depending on the
				// expanded value
				openNode : function(p_event) {
					v_tree.openNode(this);
				},
				// /// Expanding the node
				...
				// p_expanded: True or false, indicating wether the node starts
				// expanded or not;
				// p_icon: Icon;
				// p_tag: Tag;
				// p_contextmenu: Context Menu;
				createChildNode : function(p_text, p_expanded, p_icon, p_tag,
						p_contextmenu) {
					return v_tree.createNode(p_text, p_expanded, p_icon, this,
							p_tag, p_contextmenu);
				}
			}
			...
			if (this.rendered) {
				if (p_parentNode == undefined) {
					...
					var v_ul = p_parentNode.elementLi
					...
					if (p_parentNode.childNodes.length == 0) {
						if (p_parentNode.expanded) {
							...
							v_img = p_parentNode.elementLi
									.getElementsByTagName("img")[0];
							v_img.style.visibility = "visible";
							v_img.src = '/?getstatic=/folder_arr.png';
							v_img.id = 'toggle_off';
						} else {
							...
							v_img = p_parentNode.elementLi
									.getElementsByTagName("img")[0];
							v_img.style.visibility = "visible";
							v_img.src = '/?webide=1&getstatic=/folder_arr_down.png';
							v_img.id = 'toggle_on';
						}
					}
		...
		// p_ulElement: Reference to the UL tag element where the node should be
		// created;
		...
		drawNode : function(p_ulElement, p_node) {
			...
			if (p_node.childNodes.length == 0) {
				v_exp_col = createImgElement('toggle_off', 'exp_col',
						'?webide=1&getstatic=folder_arr.png');
				...
				if (p_node.expanded) {
					...
					v_exp_col = createImgElement('toggle_on', 'exp_col',
							'?webide=1&getstatic=folder_arr_down.png');
				}
			}
		...
		// /// Expanding all nodes inside the subtree that have parameter
		// 'p_node' as root
		// p_node: Subtree root;
		...
		// /// Collapsing all nodes inside the subtree that have parameter
		// 'p_node' as root
		// p_node: Subtree root;
		...
		expandNode : function(p_node) {
			if (p_node.childNodes.length > 0 && p_node.expanded == false) {
				...
				img.id = "toggle_off";
				img.src = '?webide=1&getstatic=folder_arr.png';
		...
		collapseNode : function(p_node) {
			if (p_node.childNodes.length > 0 && p_node.expanded == true) {
				...
				img.id = "toggle_on";
				img.src = '?webide=1&getstatic=folder_arr_down.png';
				elem_ul = img.parentElement.getElementsByTagName("ul")[0];
		...
		nodeContextMenu : function(p_event, p_node) {
			if (p_event.button == 2) {
				...
				if (p_node.contextMenu != undefined) {
					...
					if (p_node.childNodes.length > 0) {
						for (var i = 0; i < v_menu.foldermenu.length; i++)
							(function(i) {
								if (v_menu.foldermenu[i].type != undefined) {
									if (v_menu.foldermenu[i].type == p_node.tag
											|| v_menu.foldermenu[i].type == "Common") {
										var v_li = createSimpleElement('li',
												null, null);

										var v_span = createSimpleElement(
												'span', null, null);
										v_span.onclick = function() {
											v_menu.foldermenu[i].action(p_node)
										};

										var v_a = createSimpleElement('a',
												null, null);
										var v_ul = createSimpleElement('ul',
												null, 'sub-menu');

										v_a
												.appendChild(document
														.createTextNode(v_menu.foldermenu[i].text));

										v_li.appendChild(v_span);

										if (v_menu.foldermenu[i].icon != undefined) {
											var v_img = createImgElement(
													'null', 'null',
													v_menu.foldermenu[i].icon);
										...
										if (v_menu.foldermenu[i].submenu != undefined) {
											var v_span_more = createSimpleElement(
													'div', null, null);
											v_span_more
													.appendChild(createImgElement(
															null, 'menu_img',
															'images/right.png'));
											v_li.appendChild(v_span_more);
											v_tree
													.contextMenuLi(
															v_menu.foldermenu[i].submenu,
															v_ul, p_node);
										}
									}
								}
						...
						for (var i = 0; i < v_menu.filemenu.length; i++)
							(function(i) {
								if (v_menu.filemenu[i].type != undefined) {
									if (p_node.text.indexOf(v_menu.filemenu[i].type)==0
											|| v_menu.filemenu[i].type == "Common") {
										...
										var v_span = createSimpleElement(
												'span', null, null);
										v_span.onclick = function() {
											v_menu.filemenu[i].action(p_node)
										};
										...
										v_a
												.appendChild(document
														.createTextNode(v_menu.filemenu[i].text));

										v_li.appendChild(v_span);

										if (v_menu.filemenu[i].icon != undefined) {
											var v_img = createImgElement(
													'null', 'null',
													v_menu.filemenu[i].icon);
											v_li.appendChild(v_img);
										}
										...
										if (v_menu.filemenu[i].submenu != undefined) {
											var v_span_more = createSimpleElement(
													'div', null, null);
											v_span_more
													.appendChild(createImgElement(
															...
															'images/right.png'));
											v_li.appendChild(v_span_more);
											v_tree.contextMenuLi(
													v_menu.filemenu[i].submenu,
													v_ul, p_node);
										}
									}
								}
		...
		// /// Recursive function called when rendering context menu submenus.
		// This function should no be called directly
		...
		contextMenuLi : function(p_submenu, p_ul, p_node) {
			...
			for (var i = 0; i < p_submenu.elements.length; i++)
				(function(i) {
					...
					v_a.appendChild(document
							.createTextNode(p_submenu.elements[i].text));

					v_li.appendChild(v_span);

					if (p_submenu.elements[i].icon != undefined) {
						var v_img = createImgElement('null', 'null',
								p_submenu.elements[i].icon);
						v_li.appendChild(v_img);
					}
					...
					if (p_submenu.elements[i].p_submenu != undefined) {
						...
						v_span_more.appendChild(createImgElement(null,
								'menu_img', 'images/right.png'));
						v_li.appendChild(v_span_more);
						v_tree.contextMenuLi(p_submenu.elements[i].p_submenu,
								v_ul, p_node);
					}

				})(i);
		},
		// /// Adjusting tree dotted lines. This function should not be called
		// directly
		// p_node: Reference to the node;
